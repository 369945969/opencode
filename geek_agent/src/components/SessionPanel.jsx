...
const SessionPanel = (props) => {
    return (<div class="flex-grow p-10 overflow-y-auto bg-gradient-to-b from-transparent to-[#0A0E1A]/40 scrollbar-hide">
      <div class="max-w-6xl mx-auto space-y-10">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-10">
          <For each={props.sessions}>
            {(session) => (<div class={`glass-panel p-8 group relative overflow-hidden transition-all duration-500 hover:border-[#00F0FF]/40 border ${session.type === 'Master' ? 'border-[#00F0FF]/20' : 'border-white/5'} ${props.selectedId === session.id ? 'ring-1 ring-[#00F0FF]/60' : ''}`} onClick={() => props.onSelect && props.onSelect(session.id)}>
                <div class={`absolute -top-10 -right-10 w-40 h-40 rounded-full blur-[80px] opacity-10 transition-opacity group-hover:opacity-20 ${session.type === 'Master' ? 'bg-[#00F0FF]' : 'bg-[#FF006E]'}`}></div>

                <div class="relative z-10">
                  <div class="flex justify-between items-start mb-6">
                    <div class="flex items-center space-x-4">
                      <div class={`w-14 h-14 rounded-2xl flex items-center justify-center border transition-transform duration-500 group-hover:scale-110 ${session.type === 'Master'
                ? 'bg-[#00F0FF]/5 border-[#00F0FF]/20 text-[#00F0FF]'
                : 'bg-[#FF006E]/5 border-[#FF006E]/20 text-[#FF006E]'}`}>
                        <iconify-icon icon={session.type === 'Master' ? 'line-md:brain' : 'line-md:cog-loop'} class="text-3xl"></iconify-icon>
                      </div>
                      <div>
                  ...
                  <div class="space-y-6">
                    <div>
                      ...
                      <div class="w-full h-1.5 bg-white/5 rounded-full overflow-hidden p-[1px]">
                        <div class="h-full bg-gradient-to-r from-[#00F0FF] to-[#0090FF] transition-all duration-1000 rounded-full" style={{ width: `${session.progress}%` }}></div>
                      </div>
                    </div>
                    ...
                    <div class="flex justify-between items-center pt-2 border-t border-white/5">
                      ...
                      <div class="flex space-x-3">
                        <button class="w-9 h-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#00F0FF]/20 hover:border-[#00F0FF]/40 transition-all group/btn">
                          <iconify-icon icon="line-md:play" class="text-sm group-hover/btn:scale-110 transition-transform"></iconify-icon>
                        </button>
                        <button class="w-9 h-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#FF006E]/20 hover:border-[#FF006E]/40 transition-all group/btn">
                          <iconify-icon icon="line-md:close" class="text-sm group-hover/btn:scale-110 transition-transform"></iconify-icon>
                        </button>
                      </div>
                    </div>
                  </div>
