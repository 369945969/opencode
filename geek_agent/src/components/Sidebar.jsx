...
const Sidebar = (props) => {
    const modes = ['Analyst', 'Architect', 'Craftsman', 'QA', 'DevOps'];
    return (<aside class="w-72 border-r border-white/10 glass-panel flex flex-col p-6 space-y-8 bg-[#0D1117]/80 backdrop-blur-xl">
            <div>
                <div class="text-[10px] font-mono text-[#00F0FF]/50 uppercase tracking-[0.2em] mb-4">Cognitive Modes</div>
                <nav class="space-y-2">
                    <For each={modes}>
                        {(mode) => (<div onClick={() => props.onModeChange(mode)} class={`group flex items-center px-4 py-3 rounded-xl cursor-pointer transition-all duration-300 border ${props.currentMode === mode
